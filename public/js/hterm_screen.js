...
hterm.Screen.prototype.clearCursorRow = function() {
  this.cursorRowNode_.innerHTML = '<span/>';
  ...
  this.cursorRowNode_.firstChild.appendChild(node);
...
hterm.Screen.prototype.setCursorPosition = function(row, column) {
  ...
  var node = rowNode.firstChild.firstChild;

  if (!node) {
    node = rowNode.ownerDocument.createTextNode('');
    rowNode.firstChild.appendChild(node);
  }
...
hterm.Screen.prototype.maybeClipCurrentRow = function() {
  if (this.cursorRowNode_.firstChild.textContent.length <= this.columnCount_) {
...
hterm.Screen.prototype.insertString = function(str) {
  ...
  if (reverseOffset < 0) {
    ...
    if (!(this.textAttributes.underline || this.textAttributes.background)) {
      ...
      this.cursorRowNode_.firstChild.insertBefore(wsNode, cursorNode.nextSibling);
  ...
  if (offset == 0) {
    ...
    this.cursorRowNode_.firstChild.insertBefore(newNode, cursorNode);
  ...
  if (reverseOffset == 0) {
    ...
    var newNode = this.textAttributes.createContainer(str);
    this.cursorRowNode_.firstChild.insertBefore(newNode, nextSibling);
    this.cursorNode_ = newNode;
  ...
  var newNode = this.textAttributes.createContainer(str);
  this.cursorRowNode_.firstChild.insertBefore(newNode, cursorNode.nextSibling);
  this.cursorNode_ = newNode;
  this.cursorOffset_ = strLength;
};
