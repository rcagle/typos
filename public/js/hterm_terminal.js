...
lib.rtdep('lib.colors', 'lib.PreferenceManager',
          ...
          'hterm.Screen', 'typos.ScrollPort', 'hterm.Size', 'hterm.VT');
 ...
 */
hterm.Terminal = function(opt_profileId, div) {
  ...
  this.scrollPort_ = new typos.ScrollPort(this);
...
hterm.Terminal.prototype.appendRows_ = function(count) {
  ...
  for (var i = 0; i < count; i++) {
    ...
    var wrapper = this.document_.createElement('span');
    row.appendChild(wrapper);
    wrapper.appendChild(this.document_.createTextNode(''));
...
hterm.Terminal.prototype.eraseToRight = function(opt_count) {
  ...
  if (this.screen_.textAttributes.background ===
    ...
    if (cursorRow.firstChild.textContent.length <=
...
hterm.Terminal.prototype.syncCursorPosition_ = function() {
  ...
  var cursorRow = this.screen_.rowsArray[this.screen_.cursorPosition.row];
  this.cursorNode_.style.top = cursorRow.getBoundingClientRect().top;
...
hterm.Terminal.prototype.insertHTMLFragment = function(fragment) {
  var el = this.document_.createElement("div");
  el.innerHTML = fragment;
  el.className = 'typos-fragment';

  var pos = this.screen_.cursorPosition;
  var cs = this.scrollPort_.characterSize;
  var currentRow = this.screen_.rowsArray[pos.row];

  // Since rows are scrolled into view asynchronously, they might not
  // be attached to the document. Use a stable element to calculate
  // the screen dimensions of the fragment.
  var cheatRow = this.scrollPort_.screen_.firstChild; // Hack, fix!
  cheatRow.appendChild(el);
  el.style.cssText = 
        "position: absolute; " +
        "left: " + (pos.column * cs.width) + "px; " +
        "top: 0; " +
        "white-space: normal; ";
  var w = Math.ceil(el.scrollWidth / cs.width);
  var h = Math.ceil(el.scrollHeight / cs.height);
