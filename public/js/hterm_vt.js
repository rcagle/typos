...
hterm.VT.ESC[']'] = function(parseState) {
  ...
  function parseOSC(parseState) {
    ...
    // We're done.
    var colon = parseState.args[0].indexOf(";");
    if (colon != -1) {
      var code = parseState.args[0].substring(0, colon);
      parseState.args[0] = parseState.args[0].substring(colon + 1);
      this.dispatch('OSC', code, parseState);
    } else {
 ...
 */
hterm.VT.OSC['8080'] = function(parseState) {
  var hostHTML = parseState.args[0];
  var colon = hostHTML.indexOf(";");
  if (colon == -1) {
    // Log an error?
    console.warn("Host html had no security token.");
    return false;
  }
  var securityToken = hostHTML.substring(0, colon);
  var fragment = hostHTML.substring(colon + 1);

  // if (securityToken != this.terminal.securityToken) log and return;

  this.terminal.insertHTMLFragment(fragment);
};
